...
if [ -f "$VAULT" ]; then
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    key="${line%%=*}"; val="${line#*=}"
    case "$key" in
      SPORTSRC_API_KEY|FOOTBALL_DATA_ORG_KEY|GEMINI_API_KEY|GROQ_API_KEY|TAVILY_API_KEY|GROQ_EXTRACTION_MODEL|TELEGRAM_BOT_TOKEN|TELEGRAM_CHAT_ID|TELEGRAM_ADMIN_CHAT_ID|TELEGRAM_API_KEY|SMTP_ADMIN_EMAIL|SMTP_HOST|SMTP_PORT|SMTP_USER|SMTP_PASS|SMTP_SENDER_NAME)
...
OPTIONAL=(TELEGRAM_ADMIN_CHAT_ID TELEGRAM_API_KEY TAVILY_API_KEY GROQ_EXTRACTION_MODEL)

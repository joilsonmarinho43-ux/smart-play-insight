...
OPTIONAL=(TAVILY_API_KEY GROQ_EXTRACTION_MODEL TELEGRAM_ADMIN_CHAT_ID TELEGRAM_API_KEY SMTP_ADMIN_EMAIL SMTP_HOST SMTP_PORT SMTP_USER SMTP_PASS SMTP_SENDER_NAME)
...
if [ "${1:-}" = "--search-only" ]; then
  read -r -s -p "TAVILY_API_KEY (Enter mantém a atual): " V || V=''
  echo
  [ -n "$V" ] && setv TAVILY_API_KEY "$V"
  echo "Busca web configurada no cofre; aplique com bash deploy/fix-secrets.sh"
  exit 0
fi

FROM_ENV=0;[ "${1:-}" = '--from-env' ]&&FROM_ENV=1
for K in "${REQUIRED[@]}" "${OPTIONAL[@]}";do CUR="$(current "$K")";if [ "$FROM_ENV" -eq 1 ];then V="${!K:-}";[ -n "$V" ]&&setv "$K" "$V";continue;fi;[ -n "$CUR" ]&&echo "✓ $K já definido (${#CUR} chars) — Enter mantém"||echo "• $K — ${WHY[$K]:-opcional}";read -r -s -p "  $K: " V||V='';echo;[ -n "$V" ]&&setv "$K" "$V";done

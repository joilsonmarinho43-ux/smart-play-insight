    ...
    <Sidebar collapsible="icon" className="border-r border-white/10 bg-[#0f172a]">
      <SidebarContent className="bg-[#0f172a]">
        <div
          ...
          style={{ background: "radial-gradient(ellipse 120% 80% at 50% 0%, #1a1305 0%, #0d0a05 45%, #07070a 100%)" }}
        >
          <div className="absolute inset-0 opacity-40 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 40%, hsl(38 95% 55% / 0.18) 0%, transparent 60%)" }} />
          ...
          {!collapsed ? (
            <div className="relative z-10 flex flex-col items-center justify-center px-4 text-center">
              <div className="relative mb-3">
                <div className="absolute inset-0 blur-xl opacity-70" style={{ background: "radial-gradient(circle, hsl(40 95% 55% / 0.55) 0%, transparent 70%)" }} />
                <div className="relative h-12 w-12 rotate-45 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg, #1a1305 0%, #2a1d08 100%)", border: "1px solid hsl(40 80% 45% / 0.6)", boxShadow: "inset 0 0 12px hsl(40 95% 50% / 0.25), 0 4px 18px hsl(40 90% 40% / 0.35)" }}>
                  <span className="-rotate-45 font-display text-base font-black tracking-tight" style={{ background: "linear-gradient(180deg, #ffe08a 0%, #f5c842 50%, #c97b18 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>N33</span>
                </div>
              </div>
              <div className="flex items-baseline gap-1.5">
                <span className="font-display text-[1.6rem] leading-none tracking-[0.14em] uppercase" style={{ background: "linear-gradient(180deg, #fff5d6 0%, #f5c842 55%, #c97b18 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", filter: "drop-shadow(0 1px 6px hsl(40 90% 45% / 0.35))" }}>Nexus</span>
                <span className="font-display text-[1.6rem] leading-none tracking-[0.06em]" style={{ color: "hsl(40 92% 60%)", textShadow: "0 0 12px hsl(40 95% 50% / 0.45)" }}>33</span>
              </div>
            ...
            <div className="relative z-10 flex items-center justify-center">
              <div className="relative h-9 w-9 rotate-45 rounded-md flex items-center justify-center" style={{ background: "linear-gradient(135deg, #1a1305 0%, #2a1d08 100%)", border: "1px solid hsl(40 80% 45% / 0.6)", boxShadow: "inset 0 0 10px hsl(40 95% 50% / 0.25)" }}>
                <span className="-rotate-45 font-display text-xs font-black" style={{ background: "linear-gradient(180deg, #ffe08a 0%, #d4960a 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>N33</span>
              </div>
            </div>
          )}
        </div>
      ...
      <SidebarFooter className="bg-[#0f172a] border-t border-white/10 p-3 space-y-2">
        <div className={`flex items-center gap-1 ${collapsed ? 'justify-center' : 'justify-between'} px-1`}>
          {!collapsed && <span className="text-[10px] text-gray-500 font-medium">Zoom</span>}
          <div className="flex items-center gap-1">
            <button onClick={decrease} disabled={!canDecrease} aria-label="Diminuir tamanho da fonte" className="flex items-center justify-center w-8 h-8 rounded-lg bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ZoomOut className="h-4 w-4" /></button>
            <button onClick={increase} disabled={!canIncrease} aria-label="Aumentar tamanho da fonte" className="flex items-center justify-center w-8 h-8 rounded-lg bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ZoomIn className="h-4 w-4" /></button>
          </div>
        </div>
        <button onClick={signOut} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-red-400 hover:bg-red-500/10 transition-colors">

...
export default function MatchCard({ match: rawMatch, isPremium }: Props) {
  ...
  return <article ref={cardRef} className="overflow-hidden rounded-2xl border border-white/10 bg-[#101521]/90 shadow-2xl shadow-black/20 backdrop-blur-xl">
    ...
    <div className="grid grid-cols-2 gap-2 px-4 pb-3"><Link to={`/match/${match.id}`} onClick={() => { try { sessionStorage.setItem(`nexus-match-${match.id}`, JSON.stringify(match)); } catch {} }} className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 py-2.5 text-xs font-black text-gray-300 hover:bg-white/10"><BarChart3 className="h-4 w-4"/>Detalhes completos</Link><button onClick={()=>setReadingOpen(true)} className="flex w-full items-center justify-center gap-2 rounded-xl border border-orange-500/30 bg-orange-500/10 py-2.5 text-xs font-black text-orange-300 hover:bg-orange-500/20"><BookOpen className="h-4 w-4"/>📖 Leitura do Jogo</button></div>

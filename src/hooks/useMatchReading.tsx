...
export interface AnalystReading {
  cenario: string; pontoAtencao: string; veredito: string; risco: "baixo" | "medio" | "alto";
  aiAudit?: { status: "PASS" | "CAUTION" | "BLOCK"; reasons: string[]; evidenceQuality: "alta" | "media" | "baixa"; source: string };
  contextoDetalhado?: { desfalques?: string; arbitro?: string; clima?: string; motivacao?: string };
  mercados?: { vitoria?: string; duplaChance?: string; handicap?: string; overUnderGols?: string; btts?: string; escanteios?: string; cartoes?: string; placarExato?: string };
  oddsReferencia?: { casa?: string; empate?: string; fora?: string; over25?: string; under25?: string; bttsSim?: string; escanteiosOver9?: string; cartoesOver4?: string };
}
...
const researchCache = new Map<string, { ts: number; evidence: ResearchEvidence[] }>();
const quantCache = new Map<string, { ts: number; match: MatchData }>();
const TTL = 8 * 60 * 1000; const ANALYST_TTL = 30 * 60 * 1000; const FALLBACK_TTL = 60 * 60 * 1000; const RESEARCH_TTL = 20 * 60 * 1000; const QUANT_TTL = 6 * 60 * 60 * 1000;
...
async function enrichQuantitativeData(match: MatchData): Promise<MatchData> {
  ...
  if (cached && Date.now() - cached.ts < QUANT_TTL) return cached.match;
  try {
    ...
    const homeGames = Number(home.games ?? 0), awayGames = Number(away.games ?? 0);
    const homeGF = Number(home.goalsForAvg), awayGF = Number(away.goalsForAvg);
    const homeGA = Number(home.goalsAgainstAvg), awayGA = Number(away.goalsAgainstAvg);
    ...
    const modelData = {
      ...(match.modelData || {}), homeGoalsAvg: homeGF, awayGoalsAvg: awayGF,
      homeGoalsAgainstAvg: homeGA, awayGoalsAgainstAvg: awayGA,
      homeCornersAvg: match.modelData?.homeCornersAvg ?? null, awayCornersAvg: match.modelData?.awayCornersAvg ?? null,
      homeCardsAvg: match.modelData?.homeCardsAvg ?? null, awayCardsAvg: match.modelData?.awayCardsAvg ?? null,
      homeCornersVariance: match.modelData?.homeCornersVariance ?? null, awayCornersVariance: match.modelData?.awayCornersVariance ?? null,
      homeCardsVariance: match.modelData?.homeCardsVariance ?? null, awayCardsVariance: match.modelData?.awayCardsVariance ?? null,
      leagueAvg: match.modelData?.leagueAvg ?? 2.5, source: "team-form:ESPN/TSDB", historicalSample: sample,
      dataQuality: sample >= 5 ? "VALID" : "PARTIAL",
    } as MatchData["modelData"];
    const enriched: MatchData = { ...match, modelData, sampleSize: { homeGames, awayGames, homeWithStats: homeGames, awayWithStats: awayGames } };
    quantCache.set(key, { ts: Date.now(), match: enriched });
    return enriched;
...
export function useMatchReading(match: MatchData, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      const quantitativeMatch = await enrichQuantitativeData(match); if (cancel) return; let ctx:MatchContext|null=null;
      ...
      let fallback:FallbackStats|null=null; const fkey=String(fixtureId||`${quantitativeMatch.homeTeam}-${quantitativeMatch.awayTeam}`); const cachedF=fallbackCache.get(fkey);
      if(cachedF&&Date.now()-cachedF.ts<FALLBACK_TTL)fallback=cachedF.data;else if(fixtureId||quantitativeMatch.homeTeam&&quantitativeMatch.awayTeam){try{const {data:fdata,error:ferror}=await supabase.functions.invoke("match-stats-resolver",{body:{matchId:String(fixtureId||fkey),homeTeam:quantitativeMatch.homeTeam,awayTeam:quantitativeMatch.awayTeam,league:quantitativeMatch.league,kickoffISO:m.fixture?.date}});if(!ferror&&fdata&&!fdata.error){fallback=fdata as FallbackStats;fallbackCache.set(fkey,{ts:Date.now(),data:fallback});}}catch(e){console.warn("match-stats-resolver invoke fail",e);}}
      if(cancel)return; const fallbackHasStats=Boolean(fallback?.confidence_score&&fallback.confidence_score>0&&fallback.stats&&Object.values(fallback.stats).some(v=>v!==null&&v!==undefined&&v!=="")); const dadosInsuficientes=!reading&&!fallbackHasStats;
      const kickoffMs=m.fixture?.date?new Date(m.fixture.date).getTime():NaN; const nearKickoff=Number.isFinite(kickoffMs)&&Math.abs(kickoffMs-Date.now())<=2*60*60*1000; const contextNeedsResearch=!ctx||ctx.reliability!=="completo"||Boolean((ctx as any)?.warnings?.includes?.("injuries_unavailable")); const pesquisaWeb=dadosInsuficientes||nearKickoff||contextNeedsResearch;
      setState({loading:false,reading,context:ctx,error:null,analyst:null,analystLoading:true,analystError:null,fallback});
      const researchSignature=researchEvidence.map(e=>`${e.type}:${e.sourceType}:${e.observedAt}:${e.confidence}:${e.claim}`).join("|"); const akey=`${String(fixtureId||`${quantitativeMatch.homeTeam}-${quantitativeMatch.awayTeam}`)}:${pesquisaWeb?"research":"stats"}:${fallback?.source||"none"}:${statsSignature}:${quantitativeMatch.modelData?.historicalSample||0}:${researchSignature}`; const cachedA=analystCache.get(akey); if(cachedA&&Date.now()-cachedA.ts<ANALYST_TTL){if(!cancel)setState(s=>({...s,analyst:cachedA.data,analystLoading:false}));return;}

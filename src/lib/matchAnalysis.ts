...
/** Canonical market model: real historical samples only; Big Chances never become xG. */
...
function realXG(match: MatchData): { home: number | null; away: number | null } { const h = (match as any).homeStats?.expectedGoals ?? match.metrics?.xG?.[0]; const a = (match as any).awayStats?.expectedGoals ?? match.metrics?.xG?.[1]; return { home: typeof h === 'number' && Number.isFinite(h) && h >= 0 ? h : null, away: typeof a === 'number' && Number.isFinite(a) && a >= 0 ? a : null }; }
...
export function analyzeMarkets(match: MatchData): MarketAnalysis[] {
  ...
  const leagueRaw = Number(match.modelData?.leagueAvg ?? h.leagueAvg ?? a.leagueAvg ?? 2.5);
  const league = Number.isFinite(leagueRaw) && leagueRaw > 0 ? leagueRaw : null;
  const hGF = Number(match.modelData?.homeGoalsAvg ?? h.goalsFor ?? NaN);
  const aGF = Number(match.modelData?.awayGoalsAvg ?? a.goalsFor ?? NaN);
  const hGA = Number(match.modelData?.homeGoalsAgainstAvg ?? h.goalsAgainst ?? NaN);
  const aGA = Number(match.modelData?.awayGoalsAgainstAvg ?? a.goalsAgainst ?? NaN);
  const modelReady = n >= 3 && league !== null && [hGF, aGF, hGA, aGA].every(Number.isFinite) && hGF >= 0 && aGF >= 0 && hGA >= 0 && aGA >= 0;
  const structuralCalibration = modelReady && n >= 5 && match.modelData?.dataQuality === 'VALID' ? 'MODEL_VALIDATED' as const : 'UNCALIBRATED' as const;
  ...
  if (modelReady) {
    ...
    add(markets, 'Over 0.5 Gols', pct(poissonOver(total, 0.5)), 'Baixo', 'goals', 'MODEL_ESTIMATE', structuralCalibration);
    add(markets, 'Over 1.5 Gols', o15, o15 >= 80 ? 'Baixo' : 'Médio', 'goals', 'MODEL_ESTIMATE', structuralCalibration);
    add(markets, 'Over 2.5 Gols', o25, o25 >= 65 ? 'Médio' : 'Alto', 'goals', 'MODEL_ESTIMATE', structuralCalibration);
    if (o35 >= 20) add(markets, 'Over 3.5 Gols', o35, 'Alto', 'goals', 'MODEL_ESTIMATE', structuralCalibration);
    add(markets, 'Under 2.5 Gols', 100 - o25, 'Alto', 'goals', 'DERIVED', 'UNCALIBRATED');
    add(markets, 'Ambas Marcam', btts, btts >= 65 ? 'Médio' : 'Alto', 'btts', 'MODEL_ESTIMATE', structuralCalibration);

    const r = resultProbabilities(homeLambda, awayLambda);
    if (r.home >= 0.35) add(markets, 'Vitória Casa', pct(r.home), r.home >= 0.55 ? 'Médio' : 'Alto', 'result', 'MODEL_ESTIMATE', structuralCalibration);
    if (r.draw >= 0.28) add(markets, 'Empate', pct(r.draw), 'Alto', 'result', 'MODEL_ESTIMATE', structuralCalibration);
    if (r.away >= 0.35) add(markets, 'Vitória Fora', pct(r.away), r.away >= 0.55 ? 'Médio' : 'Alto', 'result', 'MODEL_ESTIMATE', structuralCalibration);
    if (r.home + r.draw >= 0.62) add(markets, '1X (Casa ou Empate)', pct(r.home + r.draw), 'Médio', 'chance_dupla', 'MODEL_ESTIMATE', structuralCalibration);
    if (r.away + r.draw >= 0.62) add(markets, 'X2 (Empate ou Fora)', pct(r.away + r.draw), 'Médio', 'chance_dupla', 'MODEL_ESTIMATE', structuralCalibration);
    add(markets, 'Gol no 1º Tempo', pct(1 - Math.exp(-(total * 0.45))), 'Médio', 'htft', 'MODEL_ESTIMATE', structuralCalibration);
    add(markets, 'Gol no 2º Tempo', pct(1 - Math.exp(-(total * 0.55))), 'Médio', 'htft', 'MODEL_ESTIMATE', structuralCalibration);
    ...
    if (Number.isFinite(hc) && Number.isFinite(ac) && hc >= 0 && ac >= 0 && n >= 5) {
      const lambda = bayes(hc + ac, 9.8, n, 4); const o65 = pct(poissonOver(lambda, 6.5));
      if (o65 >= 70) add(markets, 'Over 6.5 Cantos', o65, o65 >= 78 ? 'Baixo' : 'Médio', 'corners', 'MODEL_ESTIMATE', structuralCalibration);
    }
    ...
    if (Number.isFinite(hcard) && Number.isFinite(acard) && hcard >= 0 && acard >= 0 && n >= 5) {
      ...
      if (o35c >= 70) add(markets, 'Over 3.5 Cartões', o35c, o35c >= 78 ? 'Médio' : 'Alto', 'cards', 'MODEL_ESTIMATE', structuralCalibration);
    }
  }

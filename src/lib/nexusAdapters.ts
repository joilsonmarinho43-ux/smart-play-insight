...
/**
 * Compatibility adapter for legacy live payloads. It deliberately ignores
 * legacy confidence/tier values: heuristic live pressure is not a calibrated
 * probability and cannot become an official Nexus SIGNAL by adaptation.
 */
...
export function adaptHybridSignal(signal: LegacyLiveSignal, market?: MarketAnalysis): NexusDecisionOutput {
  ...
  const markets = market ? [{ ...market, probabilitySource: market.probabilitySource ?? 'HEURISTIC' as const, calibrationStatus: market.calibrationStatus ?? 'UNCALIBRATED' as const }] : [];
  ...
  return decideNexus({
    ...
    confidence: null,
    markets,
    evidence: [...liveEvidence(signal), ...marketsToNexusEvidence(markets)],

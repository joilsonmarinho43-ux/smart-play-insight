...
const observedValueCandidates = (m:MarketAnalysis[]) => signalCandidates(m).filter(x => x.oddSource === 'OBSERVED' && Number.isFinite(x.odd) && Number(x.odd) > 1 && (x.probability / 100) * Number(x.odd) - 1 > 0);
...
const selectedValidated = (m:MarketAnalysis[]) => signalCandidates(m).filter(x=>x.probability>=75).sort((a,b)=>b.probability-a.probability)[0] ?? null;
...
export function decideNexus(input:NexusDecisionInput):NexusDecisionOutput {
  ...
  if(selectedMarket?.odd != null && selectedMarket.oddSource !== 'OBSERVED') reasons.push('MARKET_ODD_SOURCE_UNVERIFIED');
  ...
  if(combinedEvidence<55)reasons.push('INSUFFICIENT_EVIDENCE'); if(!selectedMarket)reasons.push('NO_MARKET'); else if(best<75)reasons.push('MARKET_BELOW_THRESHOLD'); if(!calibratedMarket)reasons.push('NO_VALIDATED_MODEL_MARKET');
  ...
  if(confidence<70||best<75)return{decision:'INFO_ONLY',confidence:safeConfidence,riskScore:clamp(100-Math.min(safeConfidence,best)),selectedMarket,reasonCodes:reasons,evidenceScore:combinedEvidence,signalEligible:false};
  ...
  if(!valueMarket){ reasons.push('NO_POSITIVE_EV_MARKET'); return {decision:'CONSERVATIVE',confidence:safeConfidence,riskScore:clamp(100-Math.min(safeConfidence,best,combinedEvidence)),selectedMarket,reasonCodes:reasons,evidenceScore:combinedEvidence,signalEligible:false}; }

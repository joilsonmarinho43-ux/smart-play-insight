...
export function decideNexus(input:NexusDecisionInput):NexusDecisionOutput {
  ...
  if(combinedEvidence<55)reasons.push('INSUFFICIENT_EVIDENCE'); if(!selectedMarket)reasons.push('NO_MARKET'); else if(best<72)reasons.push('MARKET_BELOW_THRESHOLD'); if(!calibratedMarket)reasons.push('NO_CALIBRATED_MODEL_MARKET');

...
export function createPredictionRecord(input: Omit<PredictionRecord, 'outcome' | 'resolvedAt'>): PredictionRecord | null {
  ...
  if (!Number.isFinite(Date.parse(input.predictedAt)) || !Number.isFinite(Date.parse(input.dataObservedAt))) return null;

...
export function BingoVIPPro() {
  ...
  const rows = useMemo(() => data.map(match => ({ match, markets: analyzeMarkets(match).filter(x => x.probability >= 70 && x.probability > 0).sort((a,b) => b.probability-a.probability).slice(0,3) })).filter(x => x.markets.length).sort((a,b) => b.markets[0].probability-a.markets[0].probability).slice(0,12), [data]);
...
export function CorrectScore() {
  const { data, isLoading, refetch } = useMatches();
  const rows = useMemo(() => data.map(m=>({m,scores:exactScoreDistribution(m)})).filter(x=>x.scores.length).slice(0,16),[data]);

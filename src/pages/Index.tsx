      ...
      <main className="container max-w-3xl mx-auto px-4 py-6">
        {isFetching && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-sm text-gray-400">Processando dados da conta PRO...</p>
          </div>
        )}
        ...
        {!isFetching && matches && matches.length > 0 ? (
          ...
          <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl">
            <p className="text-gray-400">Nenhum jogo relevante para as ligas selecionadas em {date}.</p>
          </div>
        )}
      </main>

      {/* BOTÃO FLUTUANTE (ESTILO MOBILE APP) */}
      <div className="fixed bottom-8 left-0 right-0 flex justify-center z-[100] px-6">
        <Link 
          to="/live" 
          className="flex items-center gap-3 bg-orange-600 hover:bg-orange-700 text-white px-8 py-4 rounded-full shadow-[0_10px_30px_rgba(234,88,12,0.4)] transition-all active:scale-95 border border-white/20 w-full max-w-xs justify-center"
        >
          <div className="relative flex h-3 w-3">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75"></span>
            <span className="relative inline-flex rounded-full h-3 w-3 bg-white"></span>
          </div>
          <span className="font-black tracking-widest text-sm">LIVE TRADE</span>

...
export default function Live() {
  ...
  return <main className="min-h-screen p-4 sm:p-6"><div className="mx-auto max-w-6xl">
    ...
    {isLoading ? <p className="py-16 text-center text-sm text-muted-foreground">Carregando dados ao vivo…</p> : matches.length === 0 ? <div className="rounded-2xl border border-border/50 bg-card p-8 text-center text-sm text-muted-foreground">Nenhuma partida ao vivo disponível com dados reais neste momento.</div> : <div className="grid gap-4 md:grid-cols-2">{matches.map((match) => { const ls = match.liveStats; return <article key={match.id} className="rounded-2xl border border-border/50 bg-card p-4"><div className="flex items-center justify-between text-[10px] text-muted-foreground"><span>{match.league}</span><div className="flex items-center gap-2"><button type="button" onClick={() => toggleFavorite(String(match.id))} className="rounded-md p-1 text-amber-400 hover:bg-amber-500/10" aria-label={favorites.includes(String(match.id)) ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}><Star className={`h-3.5 w-3.5 ${favorites.includes(String(match.id)) ? 'fill-current' : ''}`} /></button><span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {match.minute != null ? `${match.minute}'` : match.time}</span></div></div><div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><b className="text-right text-sm">{match.homeTeam}</b><span className="rounded-lg bg-secondary px-3 py-1 text-xs font-bold">{match.liveScore ? `${match.liveScore.home}–${match.liveScore.away}` : '—'}</span><b className="text-sm">{match.awayTeam}</b></div>{ls ? <div className="mt-4 grid grid-cols-2 gap-2"><Side name="Ataques perigosos · casa" value={ls.dangerousAttacks?.[0]} /><Side name="Ataques perigosos · fora" value={ls.dangerousAttacks?.[1]} /><Side name="Escanteios · casa" value={ls.corners?.[0]} /><Side name="Escanteios · fora" value={ls.corners?.[1]} /><Side name="Posse · casa" value={ls.possession?.[0]} /><Side name="Posse · fora" value={ls.possession?.[1]} /><Side name="Pressão observada · casa" value={ls.pressureIndex?.[0]} /><Side name="Pressão observada · fora" value={ls.pressureIndex?.[1]} /></div> : <p className="mt-4 text-xs text-muted-foreground">Estatísticas ao vivo ainda não disponíveis.</p>}<p className="mt-4 text-[10px] text-muted-foreground">Índices de pressão são indicadores observacionais/heurísticos; não são probabilidade nem sinal oficial.</p></article>; })}</div>}
  </div></main>;
}

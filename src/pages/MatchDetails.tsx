...
const MatchDetails = () => {
  ...
  return <div className="min-h-screen bg-background p-4 sm:p-6 max-w-3xl mx-auto">
    ...
    <header className="mt-5 rounded-2xl border border-border/50 bg-card p-5">
      ...
      <div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><h1 className="text-right text-lg font-bold break-words">{match.homeTeam}</h1><span className="rounded-lg bg-secondary px-3 py-1 text-xs font-semibold">{match.isLive && match.liveScore ? `${match.liveScore.home}–${match.liveScore.away}` : 'VS'}</span><h1 className="text-lg font-bold break-words">{match.awayTeam}</h1></div>

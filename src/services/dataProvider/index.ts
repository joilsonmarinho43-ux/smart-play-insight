...
// fallback automático. NÃO altera a interface.
// =====================================================================

import { MatchData } from '@/types/match';
import { matchSignature } from './normalize';

export interface MatchSource { name: string; priority: number; fetchByDate: (date: string) => Promise<MatchData[]>; isAvailable?: () => boolean | Promise<boolean>; }
type ProviderLog = { ts:number; date:string; source:string; status:'ok'|'empty'|'error'; count:number; durationMs?:number; error?:string; };
const LOG_KEY='data_provider_log_v1'; const MAX_LOG_ENTRIES=50; const sources:MatchSource[]=[];
export function registerSource(s:MatchSource){const i=sources.findIndex(x=>x.name===s.name);if(i>=0)sources[i]=s;else sources.push(s);sources.sort((a,b)=>a.priority-b.priority);}
export function listSources(){return sources.map(s=>({name:s.name,priority:s.priority}));}
function pushLog(e:ProviderLog){try{const raw=localStorage.getItem(LOG_KEY);const a:ProviderLog[]=raw?JSON.parse(raw):[];a.unshift(e);localStorage.setItem(LOG_KEY,JSON.stringify(a.slice(0,MAX_LOG_ENTRIES)));}catch{}}

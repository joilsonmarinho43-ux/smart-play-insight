...
describe('Nexus adapters',()=>{
 ...
 it('mantém pré-jogo forte somente quando a calibração e odd observada estão comprovadas',()=>{const result=adaptPreMatch(strongPreMatch,[{...observedMarket,probabilitySource:'MODEL_ESTIMATE',calibrationStatus:'CALIBRATED'}],88,'CALIBRATED');expect(result.decision).toBe('SIGNAL');expect(result.signalEligible).toBe(true);expect(result.selectedMarket?.probabilitySource).toBe('MODEL_ESTIMATE');expect(result.selectedMarket?.calibrationStatus).toBe('CALIBRATED');});
 it('não promove pré-jogo sem calibração',()=>{const result=adaptPreMatch(strongPreMatch,[{...modelMarket,calibrationStatus:'UNCALIBRATED'}],88);expect(result.decision).toBe('CONSERVATIVE');expect(result.signalEligible).toBe(false);expect(result.reasonCodes).toContain('PROBABILITY_NOT_CALIBRATED');});
 it('não promove pré-jogo com amostra histórica insuficiente',()=>{const result=adaptPreMatch({...strongPreMatch,sampleSize:{homeGames:2,awayGames:5,homeWithStats:2,awayWithStats:5}},[observedMarket],88,'CALIBRATED');expect(result.decision).toBe('REJECT');expect(result.reasonCodes).toContain('DATA_INSUFFICIENT_SAMPLE');});
});

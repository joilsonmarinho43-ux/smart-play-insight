...
describe('Nexus adapters',()=>{
 ...
 it('mantém pré-jogo forte somente quando a calibração e odd observada estão comprovadas',()=>{const result=adaptPreMatch(strongPreMatch,[observedMarket],88,'CALIBRATED');expect(result.decision).toBe('SIGNAL');expect(result.signalEligible).toBe(true);expect(result.selectedMarket?.probabilitySource).toBe('MODEL_ESTIMATE');expect(result.selectedMarket?.calibrationStatus).toBe('CALIBRATED');});

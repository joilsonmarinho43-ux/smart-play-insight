...
describe('Nexus Core de Decisão', () => {
  ...
  it('não autoriza execução sem confiança explícita', () => {
  ...
  it('bloqueia execução quando há conflito entre engines', () => {
  ...
  it('autoriza somente cenário forte e consistente', () => {
    ...
    expect(out.reasonCodes).toContain('CORE_APPROVED');
  ...
  it('bloqueia partida/mercado já marcado como bloqueado', () => {
    const out = decideNexus({ match, mode: 'PRE_MATCH', confidence: 95, markets: [market], evidence: strongEvidence, executionBlocked: true });
    expect(out.decision).toBe('REJECT');
    expect(out.reasonCodes).toContain('EXECUTION_BLOCKED');
  });

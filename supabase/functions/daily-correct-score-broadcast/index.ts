...
function buildSvg(picks: Pick[], dateLabel: string): string {
  ...
  <g>
    ...
    <text x="86" y="${y + 54}" font-family="Roboto" font-size="24" font-weight="700" fill="#0b1220" text-anchor="middle">${i + 1}</text>
    <text x="126" y="${y + 42}" font-family="Roboto" font-size="30" font-weight="700" fill="#f8fafc">${svgEscape(truncate(`${p.home} x ${p.away}`, 40))}</text>
    <text x="126" y="${y + 76}" font-family="Roboto" font-size="21" fill="#94a3b8">${svgEscape(truncate(p.league, 40))} • ${svgEscape(p.date)} ${svgEscape(p.time)}</text>

    <rect x="126" y="${y + 96}" width="200" height="58" rx="12" fill="#1c2b4a"/>
    <text x="226" y="${y + 122}" font-family="Roboto" font-size="17" fill="#94a3b8" text-anchor="middle">PLACAR EXATO</text>
    <text x="226" y="${y + 146}" font-family="Roboto" font-size="24" font-weight="700" fill="#fbbf24" text-anchor="middle">${svgEscape(p.score)} • ${p.scoreProb}%</text>

    <rect x="342" y="${y + 96}" width="330" height="58" rx="12" fill="#1c2b4a"/>
    <text x="507" y="${y + 122}" font-family="Roboto" font-size="17" fill="#94a3b8" text-anchor="middle">COBERTURA 3 PLACARES</text>
    <text x="507" y="${y + 146}" font-family="Roboto" font-size="22" font-weight="700" fill="#f8fafc" text-anchor="middle">${svgEscape(p.combo.join('  /  '))} • ${p.comboProb}%</text>

    <rect x="688" y="${y + 96}" width="150" height="58" rx="12" fill="#1c2b4a"/>
    <text x="763" y="${y + 122}" font-family="Roboto" font-size="17" fill="#94a3b8" text-anchor="middle">ODD MÍN.</text>
    <text x="763" y="${y + 146}" font-family="Roboto" font-size="24" font-weight="700" fill="#f8fafc" text-anchor="middle">${p.comboOdd.toFixed(2)}</text>

    <rect x="854" y="${y + 96}" width="150" height="58" rx="12" fill="#1c2b4a"/>
    <text x="929" y="${y + 122}" font-family="Roboto" font-size="17" fill="#94a3b8" text-anchor="middle">CONFIANÇA</text>
    <text x="929" y="${y + 146}" font-family="Roboto" font-size="24" font-weight="700" fill="${conf}" text-anchor="middle">${p.confidence}% ${svgEscape(p.label)}</text>

    <text x="126" y="${y + 172}" font-family="Roboto" font-size="17" fill="#64748b">Gols esperados ${svgEscape(p.lambdas)} • amostra ${svgEscape(p.sample)} jogos</text>
  </g>`;
  }).join('');
  ...
  <text x="40" y="86" font-family="Roboto" font-size="46" font-weight="700" fill="#f8fafc">PLACAR EXATO DO DIA</text>
  <text x="40" y="128" font-family="Roboto" font-size="24" fill="#f59e0b">ANALISTA JOILSON • NEXUS 33</text>
  <text x="40" y="166" font-family="Roboto" font-size="22" fill="#94a3b8">${svgEscape(dateLabel)} • modelo Poisson bivariado + Dixon-Coles</text>
  <line x1="40" y1="188" x2="1040" y2="188" stroke="#1e2b47" stroke-width="2"/>
  ${rows}
  <text x="40" y="${H - 38}" font-family="Roboto" font-size="19" fill="#64748b">Probabilidades do modelo com dados reais das equipes. Gestão de banca: máx. 1% por entrada.</text>
</svg>`;
}

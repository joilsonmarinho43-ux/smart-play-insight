...
function scoreValue(value:any){return value==null||value===''?null:Number.isFinite(Number(value))?Number(value):null;}
function espnNormalize(e:any,teamId:string,slug:string){if(!completed(e))return null;const cs=e?.competitions?.[0]?.competitors||[];if(cs.length<2)return null;const h=cs.find((x:any)=>x?.homeAway==='home')||cs[0];const a=cs.find((x:any)=>x?.homeAway==='away')||cs[1];const hs=scoreValue(h?.score?.value??h?.score),as=scoreValue(a?.score?.value??a?.score);if(hs==null||as==null)return null;const isHome=String(h?.team?.id||h?.id)===String(teamId);return{eventId:String(e?.id||''),leagueSlug:slug,date:String(e?.date||'').slice(0,10),isHome,homeName:String(h?.team?.displayName||h?.team?.name||''),awayName:String(a?.team?.displayName||a?.team?.name||''),hs,as};}
...
async function tsdbGames(name:string,teamId?:string){
  ...
  if(!resolvedId){
    ...
    if(cachedId!==undefined) resolvedId=cachedId;
      ...
      for(const q of variants(name)){
        ...
        const target=norm(q);
        const hit=teams.find((x:any)=>norm(x?.strTeam||'')===target);
  ...
  if(resolvedId){
    ...
    for(const e of j?.results||j?.events||[]){
      const hs=scoreValue(e?.intHomeScore),as=scoreValue(e?.intAwayScore);
      if(hs==null||as==null)continue;
      const isHome=String(e?.idHomeTeam)===String(resolvedId);
      out.push({eventId:String(e?.idEvent||''),date:String(e?.dateEvent||''),isHome,homeName:String(e?.strHomeTeam||''),awayName:String(e?.strAwayTeam||''),hs,as});
    }
  }
  if(out.length<3){
    for(const q of variants(name)){
      ...
      for(const e of j?.event||[]){
        const hs=scoreValue(e?.intHomeScore),as=scoreValue(e?.intAwayScore);
        if(hs==null||as==null||!/soccer|football/i.test(e?.strSport||''))continue;
...
async function fetchSummaryStats(games:any[]){const selected=games.filter(g=>g?.eventId&&g?.leagueSlug).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,5);return await Promise.all(selected.map(async g=>{const key=`${g.leagueSlug}|${g.eventId}`,sourceUrl=`${ESPN}/apis/site/v2/sports/soccer/${encodeURIComponent(g.leagueSlug)}/summary?event=${encodeURIComponent(g.eventId)}`;const cached=summaryCache.get(key);if(cached&&Date.now()-cached.ts<TTL)return{g,stats:cached.stats,sourceUrl};const raw=await get(sourceUrl);const stats=extractSummaryStats(raw);summaryCache.set(key,{ts:Date.now(),stats});return{g,stats,sourceUrl};}));}
const tsdbStatsCache=new Map<string,{ts:number;stats:any}>();
function extractTsdbStats(raw:any){const out:any={home:{},away:{}};const mapping:Record<string,string>={'ball possession':'possession','possession':'possession','expected goals':'xG','xg':'xG','total shots':'totalShots','shots on goal':'shotsOnGoal','shots on target':'shotsOnGoal','big chances':'bigChances','corner kicks':'corners','corners':'corners','offsides':'offsides','fouls':'fouls','yellow cards':'yellowCards'};for(const row of Array.isArray(raw?.eventstats)?raw.eventstats:[]){const key=mapping[normalizeStatName(row?.strStat)];if(!key)continue;for(const side of ['home','away']){const value=parseStatValue(row?.[side==='home'?'intHome':'intAway']);if(value!=null&&value>=0&&value<=({possession:100,xG:15,totalShots:60,shotsOnGoal:40,bigChances:30,corners:35,offsides:25,fouls:50,yellowCards:20} as Record<string,number>)[key])out[side][key]=value;}}return out;}
async function fetchTsdbStats(games:any[]){const selected=games.filter(g=>/^\d+$/.test(g?.eventId||'')&&!g?.leagueSlug).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,5);return await Promise.all(selected.map(async g=>{const key=g.eventId;const cached=tsdbStatsCache.get(key);if(cached&&Date.now()-cached.ts<TTL)return{g,stats:cached.stats,sourceUrl:`${TSDB}/lookupeventstats.php?id=${key}`};const raw=await get(`${TSDB}/lookupeventstats.php?id=${key}`);const stats=extractTsdbStats(raw);tsdbStatsCache.set(key,{ts:Date.now(),stats});return{g,stats,sourceUrl:`${TSDB}/lookupeventstats.php?id=${key}`};}));}
function avg(values:number[]){return values.length?values.reduce((a,b)=>a+b,0)/values.length:null;}
function summarize(games:any[],summaries:any[]=[]){const keys=['possession','xG','totalShots','shotsOnGoal','bigChances','corners','offsides','fouls','yellowCards'];const byEvent=new Map(summaries.map((x:any)=>[`${x?.g?.leagueSlug?'espn':'tsdb'}:${x?.g?.eventId}`,x]));const score=(g:any)=>{const bucket=byEvent.get(`${g.leagueSlug?'espn':'tsdb'}:${g.eventId}`)?.stats?.[g.isHome?'home':'away']||{};return keys.filter(k=>Number.isFinite(bucket[k])).length;};const seen=new Set<string>();const sorted=games.filter(Boolean).sort((a,b)=>b.date.localeCompare(a.date)||score(b)-score(a)).filter(g=>{const k=`${g.date}|${norm(g.isHome?g.awayName:g.homeName)}`;if(seen.has(k))return false;seen.add(k);return true;}).slice(0,5);const gf:number[]=[],ga:number[]=[],recentResults:any[]=[];for(const g of sorted){const my=g.isHome?g.hs:g.as,opp=g.isHome?g.as:g.hs;gf.push(my);ga.push(opp);recentResults.push({result:my>opp?'W':my<opp?'L':'D',gf:my,ga:opp,opp:g.isHome?g.awayName:g.homeName,date:g.date});}const vals:any=Object.fromEntries(keys.map(k=>[k,[]]));const sources:any=Object.fromEntries(keys.map(k=>[k,[]]));for(const g of sorted){const result=byEvent.get(`${g.leagueSlug?'espn':'tsdb'}:${g.eventId}`);const bucket=result?.stats?.[g.isHome?'home':'away']||{};for(const k of keys)if(Number.isFinite(bucket[k])){vals[k].push(bucket[k]);if(result?.sourceUrl)sources[k].push(result.sourceUrl);}}const n=gf.length;return{games:n,goalsForAvg:n?gf.reduce((x,y)=>x+y,0)/n:0,goalsAgainstAvg:n?ga.reduce((x,y)=>x+y,0)/n:0,recentGoalsFor:gf,recentGoalsAgainst:ga,recentResults,stats:Object.fromEntries(keys.map(k=>[k,avg(vals[k])])),statsSample:Object.fromEntries(keys.map(k=>[k,vals[k].length])),statsSources:sources};}

async function form(name:string){
  // ESPN and TheSportsDB have unrelated team IDs. Resolve each provider
  // independently and start the public TSDB history while ESPN is resolving.
  const tsdbPromise=tsdbGames(name);
  ...
  const [espnStats,tsdbStats]=await Promise.all([fetchSummaryStats(games),fetchTsdbStats(games)]);const result=summarize(games,[...espnStats,...tsdbStats]);
  return result;
}

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!chatId) {
      return new Response(JSON.stringify({ success: false, disabled: true, reason: 'TELEGRAM_CHAT_ID_MISSING' }), {
        status: 200,
    ...
    const { data: existing } = await sb
      ...
      .select('id,telegram_message_id,status')
      .eq('match_id', matchId)
      .eq('market', market)
      .eq('status', 'pendente')
      .maybeSingle();
    ...
    if (existing?.id) {
      return new Response(JSON.stringify({ success: true, duplicate: true, signalId: existing.id }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const { data: inserted, error: insertError } = await sb
      ...
      .insert({
        ...
        rma_verdict: body?.rmaVerdict || null,
        rma_score: Number.isFinite(Number(body?.rmaScore)) ? Number(body.rmaScore) : null,
      })
      .select('id')
      .single();

    if (insertError) {
      return new Response(JSON.stringify({ success: false, reason: 'SIGNAL_LEDGER_INSERT_FAILED', detail: insertError.message }), {
        status: 200,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!sent.ok) {
      await enqueueTelegramOutbox(sb, {
        ...
        last_error: sent.data?.description || sent.error || `status_${sent.status}`,
      });
      return new Response(JSON.stringify({ success: false, queued: true, signalId: inserted?.id || null, reason: 'TELEGRAM_SEND_FAILED_OUTBOX' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    await sb.from('telegram_signals').update({
      telegram_message_id: sent.data?.result?.message_id || null,
    }).eq('id', inserted.id);

    return new Response(JSON.stringify({
      success: true,
      signalId: inserted.id,
      telegramMessageId: sent.data?.result?.message_id || null,
      modelProbability: probability,
      impliedProbability,
      expectedValue,
    }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
    return new Response(JSON.stringify({ success: false, reason: e instanceof Error ? e.message : 'internal_error' }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
